...
def rabotnikIndex(request):
    ...
    #sys_user = systemUser.objects.get(user_base=base_user)
    #if not request.user.is_superuser:
    #    if sys_user.position != "Работник":
...
# owner is User.position field ("Склад", "Админ" e.t.c)
#@login_required(login_url='authIndex')
def user_redirector(request,owner):
    base_user = User.objects.get(username=request.user.username)
    sys_user = systemUser.objects.get(user_base=base_user)
    if not request.user.is_superuser:
        if sys_user.position != owner:
            return HttpResponseRedirect(reverse('navigate'))
